...
class Solution1 {
    public int solution(int[] A) {
        int len = A.length;
        // peak index List
        ArrayList<Integer> pl = new ArrayList<Integer>(len / 2);
        ...
        // peak array
        Integer[] pa = pl.toArray(new Integer[0]);
        int plen = pa.length;
        if (plen < 2) {
            return plen;
        }
    ...
    public void testSingle() {
        int[] A = { 1 };
        assertEquals(0, new Solution1().solution(A));
    }
    ...
    public void testThree() {
        int[] A = { 1, 2, 1 };
        assertEquals(1, new Solution1().solution(A));
    }
